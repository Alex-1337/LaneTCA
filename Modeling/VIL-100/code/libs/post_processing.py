...
class Post_Processing(object):
    ...
    def lane_mask_generation_for_training(self, data, gt):
        ...
        for i in range(len(data)):      
            ...
            if height_idx is not None:
                ...
                h_idx2 = int(self.cfg.py_coord[height_idx[1]] / (self.cfg.height - 1) * (self.cfg.height // self.sf[0] - 1))
                lane_mask[:h_idx1] = 0
                lane_mask[h_idx2:] = 0
    ...
    def lane_mask_generation(self, data):
        ...
        for i in range(len(data)):
            ...
            height_idx = data[i]['height_idx']
            if height_idx is not None:
                h_idx1 = int(self.cfg.py_coord[np.minimum(height_idx[0] + 2, len(self.cfg.py_coord) - 1)] / (self.cfg.height - 1) * (self.cfg.height // self.sf[0] - 1))
                h_idx2 = int(self.cfg.py_coord[height_idx[1]] / (self.cfg.height - 1) * (self.cfg.height // self.sf[0] - 1))
                lane_mask[:h_idx1] = 0
                lane_mask[h_idx2:] = 0
